(() => {
  ...
  function teamTab(job) {
    ...
    return panel("Assigned team", "", `<div class="jobsDetailGrid">${job.team.map((person) => `<div class="jobsFieldCard"><div class="jobsPerson"><div class="jobsAvatar">${initials(person.name)}</div><div><b>${h(person.name)}</b><span>${h(person.employeeId)} · ${h(person.employeeId === job.lead?.employeeId ? "Lead Supervisor" : person.role)}</span></div></div></div>`).join("")}</div>`,edit);
  }
