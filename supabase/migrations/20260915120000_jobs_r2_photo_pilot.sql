-- REVIEWED APPLICATION REQUIRED: do not replay the Jobs foundation.
-- Pilot restricted to Demo Company Name and shiftly-jobs-test. No entitlement changes.
begin;
...
alter table public.job_photos
  ...
  add column byte_size integer check(byte_size > 0 and byte_size <= 2097152),
  add column sha256 text check(sha256 ~ '^[a-f0-9]{64}$');

create function public.jobs_r2_photo_context(p_company_id uuid,p_job_id uuid,p_write boolean default false)
returns jsonb language plpgsql security definer set search_path=pg_catalog,public as $$
declare a public.company_users; j public.jobs; sid uuid;
begin
  if p_company_id <> '7e7aefc5-52c2-4cc7-b649-cf255af8c2e3'::uuid then
    raise exception 'Photo pilot company required' using errcode='42501';
  end if;
  ...
  if p_write then
    ...
    if (select count(*) from public.job_photos where company_id=p_company_id and job_id=p_job_id)>=100 then
      raise exception 'Pilot limit: 100 photos per Job';
    end if;
  end if;
...
create function public.jobs_register_r2_photo(p_company_id uuid,p_job_id uuid,p_revision bigint,
...
begin
  if p_company_id is distinct from '7e7aefc5-52c2-4cc7-b649-cf255af8c2e3'::uuid then
    raise exception 'Photo pilot company required' using errcode='42501';
  end if;
  ...
  if (select count(*) from public.job_photos where company_id=p_company_id and job_id=p_job_id)>=100 then raise exception 'Pilot photo limit reached'; end if;
